...
async function getClientData(serverName: string, dbName: string, sqlText: string) {
  ...
  // `;
  const isSqlAuth = process.env.DB_USER && process.env.DB_PASSWORD;

  const conn =
    [
      `server=${serverName}`,
      `Database=${dbName}`,
      isSqlAuth ? `Uid=${process.env.DB_USER}` : `Trusted_Connection=Yes`,
      isSqlAuth ? `PWD=${process.env.DB_PASSWORD}` : null,
